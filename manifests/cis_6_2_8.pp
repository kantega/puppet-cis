...
#
# @summary 6.2.8 Ensure users own their home directories (Scored)
#
# @example
#   include cis::6_2_8
class cis::cis_6_2_8 (
  ...
  if $enforced {
    file { 'users own their home directory check script':
      path    => '/usr/local/bin/cis_6_2_8.sh',
      owner   => 'root',
      group   => 'root',
      mode    => '0711',
    ...
    if !($facts[ 'cis_6_2_8' ].empty) {
      notify { 'cis_6_2_8':
        message  => 'Not in compliance with CIS 6.2.8 (Scored). You have a home directory that is not owned by a user. Check the home_directory_owner fact for details',#lint:ignore:140chars
        loglevel => 'warning',
      }
    }
  }
