...
#
# @summary 4.1.2.3 Ensure system is disabled when audit logs are full (Scored)
#
# @example
#   include cis::4_1_2_3
class cis::cis_4_1_2_3 (
  Boolean $enforced = true,
) {

  if $enforced {

    file_line { 'auditd space_left_action':
      ensure => present,
    ...
    file_line { 'auditd action_mail_acct':
      ensure => present,
      path   => '/etc/audit/auditd.conf',
    ...
    file_line { 'auditd admin_space_left_action':
      ensure => present,
      path   => '/etc/audit/auditd.conf',
