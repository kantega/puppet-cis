...
#
# @summary 6.2.16 Ensure no duplicate GIDs exist (Scored)
#
# @example
#   include cis::6_2_16
class cis::cis_6_2_16 (
  ...
  if $enforced {
    ...
    if !($facts['cis_6_2_16'].empty) {
      notify { 'cis_6_2_16':
        message  => 'Not in compliance with CIS 6.2.16 (Scored). There are duplicate GIDs that exist in /etc/group',#lint:ignore:140chars
        loglevel => 'warning',
      }
    }
  }
