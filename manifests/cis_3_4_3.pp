...
class cis::cis_3_4_3 (
  ...
  if $enforced {
    if !$denied_networks.empty {
      # TODO format array of strings to be a valid string used in /etc/hosts.deny
      $denied_networks_real = $denied_networks
    }
    else {
      $denied_networks_real = 'ALL'
    }
    ...
    file { '/etc/hosts.deny':
      ...
      group   => 'root',
      mode    => '0644',
      content => "ALL: ${denied_networks_real}",
    }
  }
}
