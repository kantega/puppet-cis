...
#
# @summary 5.2.2 Ensure SSH access is limited (Scored)
#
# @example
#   include cis::5_2_2
class cis::cis_5_2_2 (
  ...
  if $enforced {
    ...
    if $allow_users == [] and $allow_groups == [] and $deny_users == [] and $deny_groups == [] {
      notify { 'ssh allow groups':
        message  => 'Not in compliance with CIS 5.2.2 (Scored). One or more parameters in /etc/ssh/sshd_config have not been set.',
        loglevel => 'warning',
      }
    }
    else {
