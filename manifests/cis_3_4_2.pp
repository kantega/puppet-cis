...
class cis::cis_3_4_2 (
  ...
  if $enforced {
    ...
    if !$allowed_networks.empty {
      # TODO format array of strings to be a valid string used in /etc/hosts.allow
      $allowed_networks_real = $allowed_networks
    }
    else {
      $allowed_networks_real = "${facts['network']}/${facts['netmask']}"
    }
    ...
    file { '/etc/hosts.allow':
      ...
      group   => 'root',
      mode    => '0644',
      content => "ALL: ${allowed_networks_real}",
    }
  }
}
